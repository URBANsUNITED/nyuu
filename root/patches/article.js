...
MultiEncoder.prototype = {
	...
	generate: function(data, pool, grabHeaders, obfuscateArticles) {
		...
		if (obfuscateArticles) {
			filename = crypto.randomBytes(Math.floor(Math.random() * 17) + 16).toString('hex');
		}
...
Post.prototype = {
	...
	randomizeMessageID: function() {
		...
		this.data.write(newId, this._msgIdOffset, ENCODING);
...
function RawPost(data) {
	...
	hdr.forEach(function(h) {
		...
		headers[k] = v;
		if(k == 'message-id') {
			self._msgIdOffset = p + m[1].length + m[2].length;
		}
